...
// var dbConfig = readConfig("db");
// /*jshint camelcase:false*/
// if (dbConfig.use_env_variable || !Object.keys(dbConfig).length) {
//   var dbInfo = process.env[dbConfig.use_env_variable || "DATABASE_URL"].match(
//       /([^:]+):\/\/([^:]+):([^@]+)@([^:]+):(\d+)\/(.+)/);
...
function readConfig(name) {
  var path = __dirname + "/../../config/"+name+".json";
